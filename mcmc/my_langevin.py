...
LangevinKernelResultsExtraKernelResults = collections.namedtuple(
...
class MetropolisAdjustedLangevinAlgorithm(kernel_base.TransitionKernel):
    ...
    def one_step(self, current_state, previous_kernel_results):
        ...
        with tf.control_dependencies(previous_step_size_assign):
            new_state, kernel_results = self._impl.one_step(current_state, previous_kernel_results)
            ...
            if self.step_size_update_fn is not None:
                ...
                kernel_results = kernel_results._replace(
                    extra=LangevinKernelResultsExtraKernelResults(
...
class UncalibratedLangevin(kernel_base.TransitionKernel):
    ...
    def bootstrap_results(self, init_state):
        with tf.name_scope(
            ...
            state = [tf.convert_to_tensor(x) for x in init_state_parts]
            state_parts = list(state) if mcmc_util.is_list_like(state) else [state]

            init_target_log_prob, init_grads_target_log_prob = \
                mcmc_util.maybe_call_fn_and_grads(self.target_log_prob_fn, state_parts)

            state_shape = tf.shape(init_state)
            init_volatility = tf.fill(state_shape, 1.0, name="volatility")
            init_grads_volatility = [tf.fill(state_shape, 0.0, name="grads_volatility")]

            # init_diffusion_drift = [tf.fill(state_shape, 0.0, name="diffusion_drift")]
            init_diffusion_drift = _get_drift([self.step_size], [init_volatility], init_grads_volatility, init_grads_target_log_prob)
...
def _prepare_args(target_log_prob_fn,
    ...
    if diffusion_drift is None:
        ...
        if len(state_parts) != len(diffusion_drift_parts):
...
class EmptyStepSizeAdaptation(kernel_base.TransitionKernel):
    """"""

    def __init__(self,
                 inner_kernel,
                 name=None):
        """Creates the emtpy step size adaptation kernel.
    ...
    def one_step(self, current_state, previous_kernel_results):
        ...
        new_state, new_inner_results = \
            self.inner_kernel.one_step(current_state, previous_kernel_results.inner_results)
    ...
    def bootstrap_results(self, init_state):
        ...
        return EmptyStepSizeAdaptationResults(
...
class EmptyStepSizeAdaptationResults(
    ...
    """Results of the EmptyStepSizeAdaptationResults TransitionKernel.
